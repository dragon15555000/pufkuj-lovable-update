      ...
      if (error) {
        ...
        const supabaseProducts: Product[] = data.map((p) => {
          ...
          return {
            ...
            price: p.price_grosze ?? 0,
            price_amount: p.price_grosze ?? 0,
            price_formatted: new Intl.NumberFormat("pl-PL", { style: "currency", currency: p.currency ?? "PLN" }).format((p.price_grosze ?? 0) / 100),
            price_currency: p.currency ?? "PLN",
